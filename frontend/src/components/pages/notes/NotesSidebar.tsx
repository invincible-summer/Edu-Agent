...
import {
  ChevronRight, Download, FileText, Folder, FolderPlus,
      ...
      <div key={folder.id}>
        ...
        >
          <button onClick={() => setExpanded((set) => { const next = new Set(set); if (next.has(folder.id)) next.delete(folder.id); else next.add(folder.id); return next; })} className="p-1 text-muted">
            {hasChildren ? <ChevronRight size={12} className={cn("transition-transform duration-200", open && "rotate-90")} /> : <span className="block size-3" />}
          </button>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col border-r border-border bg-surface">
        ...
        <div className="flex items-center gap-1.5">
          <div className="relative" ref={menuRef}>
            ...
            {newMenu && <div className="motion-pop absolute left-0 top-9 z-30 w-52 rounded-[10px] border border-border bg-surface p-1 shadow-lg">
              <button className="w-full rounded-md px-2 py-1.5 text-left text-xs transition-colors hover:bg-surface-hover" onClick={() => { setNewMenu(false); onCreateBlank(); }}>{tr("notes.blank")}</button>
              {templates.slice(0, 8).map((t) => <button key={t.id} className="w-full truncate rounded-md px-2 py-1.5 text-left text-xs transition-colors hover:bg-surface-hover" onClick={() => { setNewMenu(false); onCreateFromTemplate(t.id); }}>{lang === "en" && t.name_en ? t.name_en : t.name}</button>)}
            </div>}
          </div>
          <Button variant="accent2" size="sm" icon={<Sparkles size={14} />} onClick={onGenerate}>AI</Button>
        </div>
        <div className="relative"><Search size={13} className="pointer-events-none absolute left-2.5 top-1/2 z-10 -translate-y-1/2 text-muted" /><Input value={search} onChange={(e) => { onSearch(e.target.value); setPage(0); }} placeholder={tr("notes.search")} className="h-8 py-1! pl-7 pr-2 text-sm" /></div>
      </div>

      {selected.size > 0 && <div className="flex flex-wrap items-center gap-1.5 border-b border-border bg-accent-soft px-2.5 py-1.5 text-[11px] text-accent-strong">
        <span className="mr-auto">已选 {selected.size} 篇</span>
        <button className="rounded-md px-2 py-1 transition-colors hover:bg-surface" onClick={() => void pickMoveTarget()}>移动</button>
        <button className="rounded-md px-2 py-1 text-danger transition-colors hover:bg-danger/10" onClick={() => void deleteSelection()}>删除</button>
        <button className="rounded-md p-1 transition-colors hover:bg-surface" onClick={() => setSelected(new Set())}><X size={12} /></button>
      </div>}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 py-2">
        ...
        {creatingFolder ? <InlineEdit initialValue="" placeholder={tr("notes.newFolder")} onCommit={(v) => onCreateFolder(v, activeFolder || "")} onCancel={onCancelCreateFolder} /> : <button onClick={() => onCreateFolder("", activeFolder || "")} className="flex w-full items-center gap-1.5 rounded-[8px] px-2 py-1.5 text-xs text-muted transition-colors hover:bg-surface-hover hover:text-accent"><FolderPlus size={13} /> {tr("notes.newFolder")}</button>}

        {Object.keys(vault.tags).length > 0 && <div className="mt-3 border-t border-border pt-2"><div className="flex flex-wrap gap-1.5 px-1.5">{Object.entries(vault.tags).slice(0, 14).map(([tag, count]) => <button key={tag} onClick={() => { onSelectTag(activeTag === tag ? null : tag); setPage(0); }} className={cn("flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs transition-colors", activeTag === tag ? "bg-accent-soft text-accent-strong" : "bg-surface-sunken text-muted hover:bg-accent-soft hover:text-accent")}><Tag size={10} />{tag}<span className="opacity-60">{count}</span></button>)}</div></div>}
...
function FolderRow({ label, count, active, onClick }: { label: string; count: number; active: boolean; onClick: () => void }) {
  return <button onClick={onClick} className={cn("flex w-full items-center gap-1.5 rounded-[8px] px-2 py-1.5 text-xs transition-colors", active ? "bg-accent-soft font-medium text-accent-strong" : "text-fg-secondary hover:bg-surface-hover")}><ChevronRight size={12} className="opacity-50" /><span className="truncate">{label}</span><span className="ml-auto tnum text-[10px] text-muted">{count}</span></button>;
}
...
function NoteRow({ note, current, selected, lang, onSelect, onClick, onDragStart }: { note: NoteSummary; current: boolean; selected: boolean; lang: "zh" | "en"; onSelect: (event: React.MouseEvent<HTMLInputElement>) => void; onClick: () => void; onDragStart: (event: React.DragEvent<HTMLDivElement>) => void }) {
  return <div draggable onDragStart={onDragStart} className={cn("group flex items-start gap-1 rounded-[8px] px-1.5 py-1.5 transition-colors", current ? "bg-accent-soft" : "hover:bg-surface-hover")}>
